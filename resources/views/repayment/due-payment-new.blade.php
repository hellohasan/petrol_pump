                        ...
                        <div class="card-body">
                            ...
                            <div class="form-body">
                                ...
                                <div class="form-group row">
                                    <label class="col-md-12"><strong style="text-transform: uppercase;">Select Customers</strong></label>
                                    <div class="col-md-12">
                                        <select name="list_id" id="list_id" class="form-control select2 font-weight-bold text-uppercase">
                                            <option value="" class="font-weight-bold text-uppercase">Select One</option>
                                            @foreach($customer as $s)
                                                <option value="{{ $s->id }}" class="font-weight-bold text-uppercase">{{ $s->name }} - {{ $s->phone }} - Total ({{ $basic->symbol}}{{ $s->total_amount }}) - Pay ({{ $basic->symbol}}{{ $s->pay_amount }}) - Due ({{ $basic->symbol}}{{ $s->total_amount - $s->pay_amount }})</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
...
@section('scripts')
    ...
    <script>
        ...
        $('#list_id').on('change',function (e) {
            ...
            var url = '{{ url('/admin') }}';
            $.get(url + '/get-customer-due-orders?list_id=' + list_id,function (data) {
                console.log(data);
                if(data == ''){
                    ...
                    $.each(data,function (index,sub) {
                        $('#repayment_table > tbody').append('<tr><td>'+ sub.created_at+'</td><td>'+ sub.custom+'</td><td>{{ $basic->symbol }}'+ sub.order_total+'</td><td>{{ $basic->symbol }}'+ sub.pay_amount+'</td><td>{{ $basic->symbol }}'+ sub.due_amount+'</td><td><button type="button"  class="btn btn-primary btn-sm btn-detail open_modal font-weight-bold text-uppercase" value="' + sub.id + '"><i class="fa fa-credit-card"></i> Payment Now</button></td></tr>');
                    })
                }
            })
        });
