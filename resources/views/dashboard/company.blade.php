                                ...
                                <table class="table table-bordered table-hover">
                                    ...
                                    @foreach($company as $product)
                                        <tr id="product{{$product->id}}">
                                            <td>{{ $product->id }}</td>
                                            <td> {{ $product->name }}</td>
                                            <td> {{ $product->email }} <br>{{ $product->phone }}<br>{{ $product->address }}</td>
                                            <td> {{ $basic->symbol }}{{ $product->total_send }}</td>
                                            <td> {{ $basic->symbol }}{{ $product->total_pay }}</td>
                                            <td> {{ $basic->symbol }}{{ $product->total_send - $product->total_pay }}</td>
                                            <td>
                                                <button class="btn btn-primary btn-sm btn-detail open_modal font-weight-bold text-uppercase" value="{{$product->id}}"><i class="fa fa-edit"></i> Edit</button>
                                            </td>
                                        </tr>
                                    @endforeach
                                    </tbody>
        ...
        $("#btn-save").click(function (e) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    var product = '<tr id="product' + data.id + '"><td>' + data.id + '</td><td>' + data.name + '</td><td>' + data.email + ' </br>' + data.phone + '</br>'+ data.address +'</td><td>{{ $basic->symbol }}' + data.total_send + '</td><td>{{ $basic->symbol }}' + data.total_pay + '</td><td>{{ $basic->symbol }}' + due + '</td>';
                    product += '<td><button class="btn btn-primary btn-sm btn-detail open_modal font-weight-bold text-uppercase" value="' + data.id + '"><i class="fa fa-edit"></i> Edit</button></td></tr>';
