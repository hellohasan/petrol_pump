                            ...
                            <table class="table table-striped table-bordered table-hover zero-configuration">
                                ...
                                @foreach($history as $k => $p)
                                    <tr class="{{ $p->deleted_at != null ? 'bg-warning white' : '' }}">
                                        ...
                                        <td>{{ $p->custom }}</td>
                                        <td>
                                            {{$p->user->name}} <br>{{$p->user->phone}} <br>
                                            <div class="badge badge-primary font-weight-bold text-uppercase"> {{$p->user->getRoleNames()[0]}}</div>
                                        </td>
                                        <td>{{ $basic->symbol }}{{ $p->old_amount }}</td>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content" >
                ...
                <div class="modal-footer">
                    <form method="post" action="{{ route('receive.delete') }}" class="form-inline">
                        {!! csrf_field() !!}
                        {{ method_field('DELETE') }}
